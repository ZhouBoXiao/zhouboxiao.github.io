...
> AICPU 模式已经单独成篇（带术语速查和比喻，从"资源怎么过河"讲到"重复通信能不能缓存"）：[《HCCL 的 AICPU 模式：通信任务到底在哪展开》](/2026/09/23/HCCL-AICPU-mode/)。本节只保留主干结论。

### 8.1 四种通信引擎 = 四种展开位置

通信引擎的定义是 **Thread + 线程执行调度器**——Thread 承载一串数据面算子（`LocalReduce`、`ChannelRead/Write`、`Notify` 等），调度器把它们排到硬件上执行。源码里一共四种：
...
> 上：HOST 模式下 host 把算法展开成几百个 task，逐个下发，每个 2~5 μs；下：AICPU 模式 host 只提交 1 个 kernel，展开动作下沉到 AI CPU。
...
下沉之后还有两级优化，把"一次 kernel 调用"内部的开销继续压薄：

- **`BatchMode`**：`HcommBatchModeStart/End` 之间的数据面调用只生成描述符攒在本地，`End` 时才统一提交——**几百次 doorbell 压成一次**；
- **AICPU Task Cache**：同一算子反复执行时（训练里每个 step 都在跑同样的 AllReduce），把整条 SQE/WQE 序列录下来，第二次只"刷新地址 + token + pi"后重放——**连展开都省掉**。注意缓存键里含 `inputSize`，数据量一变就必然 miss，所以动态 shape 场景建议关闭缓存（`AICPU_CacheDisable`）。
...
AICPU 这条路上具体是怎么把交互次数压到 1 次的——资源怎么过河、攒批怎么落地、缓存怎么刷地址——见独立成篇的那一篇。
