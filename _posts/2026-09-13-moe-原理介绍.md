...
- $N$：专家数量。
- $i$：第$i$个专家。
- $f_i$ ：实际路由到专家 $i$ 的 token 比例，也叫硬路由频率。  例如一个 batch 有 $T$ 个 token，其中 $T_i$ 个被路由到专家 $i$，则：$f_i = \frac{T_i}{T}$，所以$\sum_i f_i = 1$。
  
- $P_i$：门控网络分配给专家 $i$ 的平均概率，也叫软路由概率。  

  $P_i = \frac{1}{T}\sum_{t=1}^{T} p_{t,i}$

  其中 $p_{t,i}$ 是第 $t$ 个$ token$ 被门控分配给专家 $i$ 的概率。  
  通常$\sum_i P_i = 1$。

- $\alpha$：辅助损失的权重系数，控制负载均衡强度。Switch Transformer 中常用$\alpha=0.01$左右。

 **直观意义**


$\sum_{i=1}^{N} f_i P_i$表示“实际负载”和“门控倾向”的点积。

如果某个专家：

- 实际接收的 token 很多，$f_i$ 大；
- 门控又倾向于把 $token$ 给它，$P_i$ 大；

那么$f_i P_i$ 就很大，辅助损失也大。

因此最小化$L_{aux}$ 会迫使门控网络不要把大量 token 集中到少数专家，而是尽量均匀分配到所有专家。
...
乘 $N$ 是为了归一化，使均匀分布时的辅助损失基准为 $\alpha$，不随专家数量变化。

如果所有 token 都路由到一个专家，则：

$$
f_k \approx 1,\quad P_k \approx 1
$$

于是：

$$
L_{aux} \approx \alpha N
$$
...
$f_i$ 来自 argmax 路由，通常是不可微的；而$P_i$是门控概率，可微。

把 $f_i$ 当作常数，则：

$$
\frac{\partial L_{aux}}{\partial P_i} = \alpha N f_i
$$

也就是说，某个专家实际接收的 token 越多，$f_i$越大，它对应的 $P_i$就会受到越大的梯度，被推低。由于 softmax 概率总和为 1，压低高负载专家的概率，就会相对提高其他专家的概率，从而实现负载均衡。
